...
static thread_action_func recent_cpu_update;
...
{
	//static int count = 0;
	//printf("Thread start is called %d \n", count++);
			   ...
			   thread_func *function, void *aux) 
{
  ...
  ef = (struct switch_entry_frame *)alloc_frame (t, sizeof *ef);
  ef->eip = (void (*) (void)) kernel_thread;

  /* Stack frame for switch_threads(). */
  sf = (struct switch_threads_frame *)alloc_frame (t, sizeof *sf);
  sf->eip = switch_entry;
  sf->ebp = 0;
...
thread_exit (void) 
{
  ASSERT (!intr_context ());
	//static int count = 1;
	//printf(" # thread_exit is called for = %d --- tid = %d \n", count++, thread_current()->tid);
#ifdef USERPROG
  process_exit ();
#endif
...
{
	//printf("thread_foreach is called **************");
  struct list_elem *e;
...
/* Sets the current thread's priority to INITIAL_PRIORITY. */
void
...
bool priority_list_less (const struct list_elem *a,
							 const struct list_elem *b,
							 void *aux UNUSED) {
